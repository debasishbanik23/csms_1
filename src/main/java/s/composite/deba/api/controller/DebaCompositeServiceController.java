package s.composite.deba.api.controller;
...
@RestController
@Api("account-service")
public class DebaCompositeServiceController {

  private static final Logger LOG = LoggerFactory.getLogger(DebaCompositeServiceController.class);
  @Autowired private Environment env;
  @Autowired private IDebaCompositeService debaCompositeService;
  ...
  public ResponseEntity<List<Account>> getAccounts(
    ...
    LOG.info(env.getProperty("account-service.log.message.key.1"));
    List<Account> data =
        (List<Account>)
            debaCompositeService
                .getAccounts(new ServiceRequestContext(headers, pathParams, queryParams))
                .getBody();
    if (!Objects.isNull(data) && !data.isEmpty()) {
      LOG.info(env.getProperty("account-service.log.message.key.2"));
      return new ResponseEntity<List<Account>>(data, HttpStatus.OK);
    } else {
      LOG.info(env.getProperty("account-service.log.message.key.3"));
  ...
  public ResponseEntity<Account> getAccount(
    ...
    LOG.info(env.getProperty("account-service.log.message.key.4"));
    Account data =
        (Account)
            debaCompositeService
                .getAccount(new ServiceRequestContext(headers, pathParams, queryParams))
                .getBody();
